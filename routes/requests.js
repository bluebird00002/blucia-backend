...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      sendEmail(
        email,
        emailTemplates.serviceRequestReceived(name, request.id)
      ).catch((err) => {
        console.error(
          "Failed to send service request confirmation email:",
          err
        );
      });
      ...
      return res.status(201).json({
        message:
          "Service request submitted successfully. A confirmation email has been sent.",
        request,
      });
    } catch (error) {
